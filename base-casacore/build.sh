...
check_prerequisites() {
    ...
    local required_files=("requirements.txt" "config.py" "ImageMagick-7_on_setonix_policy.xml")
...
setup_buildx() {
    ...
    if ! docker buildx inspect casacore-builder >/dev/null 2>&1; then
        log_info "Creating new buildx builder..."
        docker buildx create --name casacore-builder --use
    else
        log_info "Using existing buildx builder..."
        docker buildx use casacore-builder
    fi
