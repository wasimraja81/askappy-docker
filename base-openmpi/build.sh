...
#####################################################################
# Build Script for base-openmpi - Ubuntu 24.04 with MPICH only
#####################################################################
...
check_prerequisites() {
    ...
    # No required files for base-openmpi build
...
setup_buildx() {
    ...
    if ! docker buildx inspect openmpi-builder >/dev/null 2>&1; then
        log_info "Creating new buildx builder..."
        docker buildx create --name openmpi-builder --use
    else
        log_info "Using existing buildx builder..."
        docker buildx use openmpi-builder
    fi
...
main() {
    ...
    log_info "Building base-openmpi image: ${IMAGE_NAME}:${BASE_TAG}"
    log_info "Build date: ${BUILD_DATE}"
    log_info "VCS ref: ${VCS_REF}"
}
